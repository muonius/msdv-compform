...
*/
  ...
  class Sprite {
    ...
    _syncAnimationSizes() {
      ...
      if (
        ...
        this._internalWidth =
          this.animations[this.currentAnimation].getWidth() *
          abs(this._getScaleX());
        this._internalHeight =
          this.animations[this.currentAnimation].getHeight() *
          abs(this._getScaleY());
    ...
    update() {
      if (!this.removed) {
        ...
        this.newPosition = this.p.createVector(
          this.position.x,
          this.position.y
        );
        ...
        if (this.collider) {
          if (this.collider instanceof AABB) {
            ...
            if (this.colliderType === "custom") {
              ...
              this.collider.extents.x =
                this._internalWidth * abs(this.p.cos(t)) +
                this._internalHeight * abs(this.p.sin(t));

              this.collider.extents.y =
                this._internalWidth * abs(this.p.sin(t)) +
                this._internalHeight * abs(this.p.cos(t));
            }
          }
    ...
    setDefaultCollider() {
      ...
      if (
        ...
        this.collider = new AABB(
          this.p,
          this.position,
          this.p.createVector(this._internalWidth, this._internalHeight)
        );
        //quadTree.insert(this);
        this.colliderType = "default";
      }
    ...
    mouseUpdate() {
      ...
      if (this._drawnWithCamera)
        mousePosition = this.p.createVector(
          this.p.camera.mouseX,
          this.p.camera.mouseY
        );
      ...
      if (this.collider) {
        ...
        //global p5 var
        if (this.mouseIsOver && this.p.mouseIsPressed)
    ...
    setCollider(type, offsetX, offsetY, width, height) {
      ...
      if (type === "rectangle" && arguments.length === 1) {
        ...
        this.collider = new AABB(
          this.p,
          this.position,
          this.p.createVector(width, height),
          v
        );
    ...
    display() {
      if (this.visible && !this.removed) {
        ...
        this.p.scale(
          this._getScaleX() * this.dirX,
          this._getScaleY() * this.dirY
        );
        if (this.p._angleMode === this.p.RADIANS) {
    ...
    addImage() {
      if (typeof arguments[0] === "string" && arguments[1] instanceof p5.Image)
        ...
        throw new TypeError(
          "only accepts a p5.image or an image label string followed by a p5.image)"
        );
    }
    ...
    AABBops(type, target, callback) {
      ...
      if (target instanceof Sprite) others.push(target);
        ...
        throw new Error(
          "overlap can only be checked between sprites or groups"
        );
    ...
    collide(other) {
      if (this.overlap(other)) {
        ...
        var displacement = this.p.createVector(
          this.p.cos(a) * intersection,
          this.p.sin(a) * intersection
        );
  ...
  class AABB {
    ...
    rotate(r) {
      ...
      var w2 =
        this.extents.x * abs(this.p.cos(t)) +
        this.extents.y * abs(this.p.sin(t));
      var h2 =
        this.extents.x * abs(this.p.sin(t)) +
        this.extents.y * abs(this.p.cos(t));
    ...
    collide(other) {
      if (other instanceof AABB) {
        ...
        if (
          ...
          var boundsPoint = md.closestPointOnBoundsToPoint(
  ...
  class SpriteAnimation {
    constructor(pInst) {
      ...
      if (
        ...
        if (ext1 !== ".png") {
          this.p.print(
            "SpriteAnimation error: you need to use .png files (filename " +
              from +
              ")"
          );
          from = -1;
    ...
    update() {
      ...
      if (
        this.onComplete &&
        ((this.targetFrame == -1 && this.frame == this.images.length - 1) ||

...
(function () {
      ...
      switch (c) {
        ...
        case "]":
          ...
          if (lvl === 0) {
            ...
            if (inPre) prefxns.push(parseAction(section));
            else postfxns.push(parseAction(section));
            start = i + 1;
          }
          break;
  ...
  function parseRule(rule) {
    ...
    function createSection(end) {
      ...
      if (section.length > 0) {
        if (inTag) sections.push(parseTag(section));
  ...
  Rule.prototype.getParsed = function () {
    if (!this.sections) this.sections = parseRule(raw);
  ...
  Action.prototype.activate = function () {
    ...
    if (parsed.symbol) {
      ...
      } else throw "Unknown action: " + parsed.symbol;
    }
  ...
  var universalModifiers = {
    ...
    a: function (s) {
      if (!isConsonant(s.charAt())) return "an " + s;
      return "a " + s;
    },
  ...
  var TagNode = ExpansionNode.extend({
    ...
    expand: function () {
      if (tracery.outputExpansionTrace) console.log(r.sections);
  ...
  Symbol.prototype.toHTML = function (useSpans) {
    ...
    if (useSpans)
      keySpan =
        "<span class='symbol symbol_" + this.key + "'>" + keySpan + "</span>";
    ...
    var rules = this.baseRules.rules.map(function (rule) {
      ...
      var s = '"' + cleaned + '"';
      if (useSpans) s = "<span class='rule'>" + s + "</span>";
      return s;
    });
  ...
  Grammar.prototype.addOrGetSymbol = function (key) {
    if (this.symbols[key] === undefined) this.symbols[key] = new Symbol(key);
